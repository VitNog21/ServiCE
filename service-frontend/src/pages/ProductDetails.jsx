...
export default function ProductDetails() {
  ...
  const handleStartChat = () => {
    ...
    if (currentUser.id === produto.owner_id) {
      toast.info('Você não pode conversar com você mesmo.');
      return;
    }
  ...
  const handleCompra = async () => {
    ...
    if (error) {
      ...
      toast.success('Pedido realizado! Você será redirecionado para os seus anúncios.');
  ...
  const handleReport = async () => {
    if (!currentUser) {
      toast.info('Você precisa estar logado para denunciar um anúncio.');
      navigate('/login');
      return;
    }

    if (reportReason === 'Outro' && !reportDetails.trim()) {
      toast.info('Por favor, detalhe o motivo da denúncia.');
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 w-full">
        <div className="text-center space-y-4">
          ...
          <Button 
            onClick={() => navigate('/')}
            className="mt-4 bg-[#0A847C] hover:bg-[#085a51] text-white rounded-xl px-6 py-2"
          >
            Voltar à Home
          </Button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-12 w-full flex flex-col font-sans">
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '32px 24px', flex: 1, width: '100%' }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-start justify-center">
                ...
                {fotos.map((foto, index) => (
                  <Button
                    ...
                    className={`flex-shrink-0 w-24 h-24 rounded-2xl overflow-hidden border-2 transition-all p-0 bg-white ${
                      index === fotoAtual ? 'border-[#0A847C] opacity-100 ring-2 ring-[#0A847C]/20' : 'border-slate-200 opacity-60 hover:opacity-100 hover:border-[#0A847C]/50'
                    }`}
                  >
          ...
          <div className="lg:col-span-5 xl:col-span-4 w-full">
            {/* O flex flex-col e gap-8 organiza os elementos mantendo um distanciamento perfeito */}
            <div className="bg-white p-8 rounded-3xl shadow-lg shadow-slate-200/50 border border-slate-200 sticky top-28 flex flex-col gap-8">
              ...
              {/* Botões de Ação com gap-4 */}
              <div className="flex flex-col gap-4">
                <Button 
                  onClick={handleCompra} 
                  disabled={comprando}
                  className="w-full h-16 text-lg font-bold bg-[#10B981] hover:bg-[#059669] text-white transition-all rounded-2xl shadow-md hover:shadow-lg flex items-center justify-center gap-3"
                >
                ...
                <Button 
                  ...
                  className="w-full h-16 text-lg font-bold bg-white border-2 border-[#0A847C] text-[#0A847C] hover:bg-[#0A847C] hover:text-white transition-all rounded-2xl shadow-sm flex items-center justify-center gap-3"
                >
      ...
      {/* MODAL DE DENÚNCIA - TOTALMENTE CENTRALIZADO */}
